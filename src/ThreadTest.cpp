...
AA8::AA8() {
  ...
  // thread t1{ func8b, ref(*this) }; //if the Constructor in class, this line
  // can not see func8b;
  t1.detach();
...
void func9worker(int id, std::condition_variable& cv, std::mutex& m) {
  ...
  while (true) {
    {
      cout << "----------------------  id[" << id
           << "] after create lock. lk.owns_lock()=" << lk.owns_lock() << endl;

      auto check = [&] {
        int n = random();
        cout << "id[" << id << "] awake. check[" << n
             << "]. lk.owns_lock()=" << lk.owns_lock() << endl;
        return n < 30;
      };
      for (auto i : vector<int>{1, 2, 3, 4, 5}) {
        cout << "----------------------  id[" << id
             << "] before wait. owns_lock()=" << lk.owns_lock() << endl;
        std::this_thread::sleep_for(1000ms);
      }

      cout << "----------------------  id[" << id
           << "] before lock. owns_lock()=" << lk.owns_lock() << endl;
      lk.lock();
      cout << "id[" << id << "] after lock. owns_lock()=" << lk.owns_lock()
           << endl;
      cout << "id[" << id << "] goto wait." << endl;

      if (cv.wait_for(lk, 10ms, check)) {
        cout << "id[" << id
             << "] out wait success. owns_lock()=" << lk.owns_lock() << endl;
        for (auto i : vector<int>{1, 2, 3, 4, 5}) {
          cout << "id[" << id
               << "] keep success. owns_lock()=" << lk.owns_lock() << endl;
          cv.notify_all();
          std::this_thread::sleep_for(1000ms);
        }
        cout << "id[" << id
             << "] finish success. owns_lock()=" << lk.owns_lock() << endl;
      } else {
        cout << "id[" << id << "] wait timeout" << endl;
        for (auto i : vector<int>{1, 2, 3, 4, 5}) {
          cout << "id[" << id
               << "] keep timeout. owns_lock()=" << lk.owns_lock() << endl;
          cv.notify_all();
          std::this_thread::sleep_for(1000ms);
        }
        cout << "id[" << id
             << "] finish timeout. owns_lock()=" << lk.owns_lock() << endl;
      }
      cout << "id[" << id << "] before unlock. owns_lock()=" << lk.owns_lock()
           << endl;
      lk.unlock();
      cout << "----------------------  id[" << id
           << "] after unlock. owns_lock()=" << lk.owns_lock() << endl;
      cv.notify_all();
    }
